...
-- 2. Add is_protected column to positions (if not already there)
ALTER TABLE positions ADD COLUMN IF NOT EXISTS is_protected TINYINT(1) NOT NULL DEFAULT 0;
...
-- Mark existing protected posts as protected (in case they were already there)
UPDATE positions SET is_protected = 1 WHERE name IN ('Président', 'Trésorier', 'Secrétaire Général');
...
-- 5. Seed default module access for all existing users (time, absences, expenses, documents)
...
CREATE TABLE IF NOT EXISTS validation_config (
  config_type VARCHAR(50) PRIMARY KEY,             -- 'budget' | 'expenses'
